...
function toonMenu(menu, actie = 0){
    if(actie == "backup"){
        $.ajax({
            type: "GET",
            url: "/" + APP_PREFIX + "minidagboek/backups/initialise",
            data: '',
...
function toggleDropdownMaand(input){
    ...
    if(inhoud.style.display == "block"){
        input.src = "/" + APP_PREFIX + "images/expand.png"
        inhoud.style.display =  "none";
    }
    else{
        input.src = "/" + APP_PREFIX + "images/expand_reverse.png"
...
function selectBackup(input){
    ...
    if($(input).find(".check").css("background-image").indexOf("checkmark_vol") == -1){
        $(input).find(".check").css("background-image", "url('/" + APP_PREFIX + "images/checkmark_vol.png");
        $(input).css("background-color", "rgb(214, 214, 214)");
        var backup = input.id.substring(7, input.id.length);
        selectedbackups.push(backup);
    }
    else{
        $(input).find(".check").css("background-image", "url('/" + APP_PREFIX + "images/checkmark_leeg.png");
...
function deleteVinkje(input, type){
    if(type == "backup"){
        $(document.getElementsByName(input)[0]).find(".check").css({"background-image": "url('/" + APP_PREFIX + "images/checkmark_leeg.png')",
    "opacity": "0"});
    }
    else if(type == "dupError"){
        input.css({"background-image": "url('/" + APP_PREFIX + "images/checkmark_leeg.png')"});
    }
}
function addVinkje(input, type){
    if(type == "backup"){
        $(document.getElementsByName(id)[0]).find(".check").css({"background-image": "url('/" + APP_PREFIX + "images/checkmark_vol.png')","opacity": "1"});
    }
    else if(type == "dupError"){
        input.css({"background-image": "url('/" + APP_PREFIX + "images/checkmark_vol.png')"});
    }
}
...
function toggleBackup(input){
    ...
    if(inputId.length == 5){
        ...
        //ALS DE MAAND VOL IS
            ...
            var maanddagen = $(maandklasse);
            for(var i = 0; i < maanddagen.length; i++){
                $(maanddagen[i]).find(".check").css("background-image", "url('/" + APP_PREFIX + "images/checkmark_leeg.png')");
...
function verzendEntry(){
    ...
    //ERROR 
    if(!geldigeDatum || !geldigeWeekdag || !geldigeMuziek){
        $("#errorcont").html("");
        if(!geldigeDatum){
        ...
        $.ajax({
            type: "GET",
            url: "/" + APP_PREFIX + "minidagboek/addEntry",
...
function verzendEntries(){
    ...
    $("#KBM_button").html("<div id='ajaxSpinner'></div>");
    var target = document.getElementById('ajaxSpinner');
    var spinner = new Spinner(opts).spin(target);
    $.ajax({
        type: "POST",
        url: "/" + APP_PREFIX + "minidagboek/addEntries",
        data: {'entries' : KBM},
        success: function(data) {
            $("#KBM_button").html("Upload");
            if(data.length > 0){
                var tekst = "<div id='errorDuplicateEntryCont'>";
                for(var i = 0; i < data.length; i++){
                    tekst += "<div class='errorDuplicateEntry'><div class='errorDuplicateEntryCheck' onclick='addSelectedDuplicate(this)'></div><tekst>" + data[i] + "</tekst></div>";
...
function selectAllDuplicates(input){
    ...
    if(muiselem.css("background-image").indexOf("checkmark_leeg") == -1){
        ...
        var allEntries = $(".errorDuplicateEntryCheck");
...
function overrideDuplicates(){
    ...
    $.ajax({
        url: "/" + APP_PREFIX + "minidagboek/overrideDuplicates",
        type: "POST",
...
$(document).ready(function() {
    $.ajax({
      url: "/" + APP_PREFIX + "minidagboek/initialise",
      type: 'GET',
      data: '',
      success: function(data){
...
$(document).ready(function() {
    ...
    $("body").click(function(e){
        if(openmenu > 0){
            var muiselem = e.target;
            if(muiselem.className == "togglepopup") {
                hideMenu(muiselem.id);
                
                
                //backup popup resetten
                if(muiselem.id == "backpopup"){
                    backupitems = [];
                    selectedbackups = [];
                    $("#backselectcont").html("");
                    var allechecks = $("#backupitemscont3").find(".check");
                    for(var i = 0; i < allechecks.length; i++){
                        $(allechecks[i]).css({"background-image": "url('/" + APP_PREFIX + "images/checkmark_leeg.png')",
    "opacity": "0"});
                    }
    ...
    document.addEventListener('contextmenu', function(e) {
        ...
        if(wat != ""){
            ...
            var inhoud = "<li class='RKli'><fakeA id='RKedit' class='RKa'><span class='helper'></span><img  class='RKimg' src='/" + APP_PREFIX + "images/edit-zwt.png'>Bewerk</fakeA></li>"
                    + "<li class='RKli'><fakeA id='RKdelete' class='RKa'><span class='helper'></span><img class='RKimg' src='/" + APP_PREFIX + "images/delete-zwt.png'>Verwijder</fakeA></li>"
                    + "<li class='RKli'><fakeA id='RKstat' class='RKa'><span class='helper'></span><img class='RKimg' src='./" + APP_PREFIX + "images/statistieken-zwt.png'>Statistieken</fakeA></li>";   
            $("#RKul").html(inhoud);
